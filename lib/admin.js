...
function getGroups(callback) {
    const ephemeralGroups = groups.getEphemeralGroups();
...
const can = function (mode, uid, tid, callback) {
    topics.getTopicField(tid, 'cid', function (err, cid) {
        ...
        groups.isMemberOfGroups([uid], allowedGroups, function (err, areMembers) {
            let isMember = areMembers[0];
            isMember = isMember || (allowedGroups.indexOf('guests') !== -1);
