...
function parsePluginSettings(rawSettings) {
    return {
        'allowed-categories': (rawSettings['allowed-categories'] || '')
            ...
            .filter(identity),
        'allowed-groups': (rawSettings['allowed-groups'] || '')
            .split(',')
...
module.exports.getAllowedCategories = function ()/*: Array<number>*/ {
    if (!pluginSettings['allowed-categories']) {
        winston.warn('empty "allowed-categories"!');
    }
    return pluginSettings['allowed-categories'] || [];
};

module.exports.getAllowedGroups = function ()/*: Array<string>*/ {
    if (!pluginSettings['allowed-groups']) {
        winston.warn('empty "allowed-groups"!');
    }
    return pluginSettings['allowed-groups'] || [];
};
