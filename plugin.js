...
module.exports.addDataToTopic = function (data, cb) {
    ...
    data.topics.forEach(function (topic) {
        if (isMission(topic.title || "")) {
            ...
            //firm commitments
            db.sortedSetCard((['tid', tid, 'firm_commitment'].join(':')), function (err, result) {
                topic.firm_commitments = result;
            });
...
module.exports.onLoadTopic = function (data, cb) {
    ...
    db.getSortedSetRangeWithScores((['tid', tid, 'firm_commitment'].join(':')), 0, -1, function (err, result) {
        ...
        db.getObjects(keys, function (err, users) {
            ...
            topicObj.AttendingCount = users.length;
        });
    });
    ...
    db.getSortedSetRangeWithScores((['tid', tid, 'commitment'].join(':')), 0, -1, function (err, result) {
        ...
        db.getObjects(keys, function (err, users) {
            ...
            topicObj.maybeCount = users.length
        });
    });
    ...
    db.getSortedSetRangeWithScores((['tid', tid, 'cancellation'].join(':')), 0, -1, function (err, result) {
        ...
        db.getObjects(keys, function (err, users) {
            ...
            topicObj.nonAttendingCount = users.length
        });
    });


    /*
     var arr = topicObj.Attending;
     var length = Array.isArray(arr);
     topicObj.AttendingCount = length;
     topicObj.MaybeCount = topicObj.Maybe.length;
     topicObj.nonAttendingCount = topicObj.nonAttending.length;
     */
    cb(null, data);
};
